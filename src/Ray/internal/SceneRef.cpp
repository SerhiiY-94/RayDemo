...
uint32_t Ray::Ref::Scene::AddMesh(const mesh_desc_t &_m) {
    ...
    if (use_wide_bvh_) {
        size_t mtri_count = (tri_indices_.size() / 8) + ((tri_indices_.size() % 8) ? 1 : 0);
        mtris_.resize(mtri_count);
        for (uint32_t i = tri_index_start; i < (uint32_t)tri_indices_.size(); i++) {
            const tri_accel_t &tri = tris_[tri_indices_[i]];
            mtris_[i / 8].init(tri, (i % 8));
        }

        if (tri_indices_.size() % 8) {
            mtri_accel_t &mtri = mtris_[mtri_count - 1];
            for (int i = int(tri_indices_.size() % 8); i < 8; i++) {
                mtri.init(InvalidTriangle, i);
            }
        }
    }
